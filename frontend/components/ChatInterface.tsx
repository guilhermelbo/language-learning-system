...
import { PronunciationEventCard, PronunciationEvent } from "./PronunciationEventCard";
    ...
    >
      ...
      {messages.map((msg) => (
        <div key={msg.id} className="space-y-2">
          ...
          {/* Pronunciation event cards — shown under assistant turns only */}
          {msg.role === "assistant" &&
            msg.pronunciationEvents &&
            msg.pronunciationEvents.length > 0 && (
              <div className="flex justify-start">
                <div className="max-w-[80%] w-full space-y-2 pl-2">
                  {msg.pronunciationEvents.map((ev, i) => (
                    <PronunciationEventCard key={`${msg.id}-ev-${i}`} event={ev} />
                  ))}
                </div>
              </div>
            )}
